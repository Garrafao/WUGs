...
def plot_graph_interactive(G, outDir, c2n, threshold=0.5, normalization=lambda x: x, color='colorful', period='full',
                           ...
                           node_label='cluster', edge_label_style='weight', edge_width=2, k=0.8, is_spring=False,
                           seed=0, pos={}, noise_color='k'):
    """
    ...
    if period == 'full':
        ...
        nodes = [node for node in G.nodes() if
                 G.nodes()[node]['type'] == 'sense' or G.nodes()[node]['grouping'] == period]
    ...
    if mode == 'pos':
        ...
        conflicts = get_annotator_conflicts(G, annotators, non_value=0.0, threshold=threshold,
    ...
    for n, cluster in c2n.items():
        ...
        for node in cluster:
            ...
            if type_ == 'usage':
                ...
                # print(text).blah
                meta_text = '<br>'.join(
                    [col + ': ' + node_data[col] for col in ['date', 'grouping', 'identifier', 'description', 'type'] if
                     (node_data[col] != '-' and node_data[col] != ' ' and node_data[col] != '')])
            ...
            if type_ == 'sense':
                ...
                meta_text = '<br>'.join([col + ': ' + node_data[col] for col in ['type'] if
                                         (node_data[col] != '-' and node_data[col] != ' ' and node_data[col] != '')])
                text = text + '<p style="font-size:8px;"><br>' + meta_text + '</p>'
            ...
            G_int.add_node(n_id=node, label=label, shape='circle', size=node_size, physics=False, x=x, y=y, color=color,
                           title=text, date=date)

    for (i, j) in elarge:
        try:
            label = make_edge_label((i, j), G, annotators, edge_label_style=edge_label_style,
                                    normalization=normalization, summary_statistic=summary_statistic)
            G_int.add_edge(i, j, color='black', width=edge_width * 2, label=label,
    ...
    for (i, j) in esmall:
        try:
            label = make_edge_label((i, j), G, annotators, edge_label_style=edge_label_style,
                                    normalization=normalization, summary_statistic=summary_statistic)
            G_int.add_edge(i, j, color='lightgray', width=edge_width * 1, label=label,
                           weight=normalization(G[i][j]['weight']))
        except (AssertionError, KeyError) as e:  # continue in case node is not in time period
            pass
    ...
    for (i, j) in enan:
        try:
            label = make_edge_label((i, j), G, annotators, edge_label_style=edge_label_style,
                                    normalization=normalization, summary_statistic=summary_statistic)
            G_int.add_edge(i, j, color='lightgray', width=edge_width * 0.5, label=label,
                           weight=normalization(G[i][j]['weight']))
        except (AssertionError, KeyError) as e:  # continue in case node is not in time period
            pass
...
def plot_graph_static(G, outDir, c2n, threshold=0.5, normalization=lambda x: x, color='colorful', period='full',
                      ...
                      is_spring=False, seed=0, is_edge_labels=False, edge_label_style='weight', dpi=300,
    ...
    if is_edge_labels:
        nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, label_pos=0.5, font_size=font_size_edges,
                                     font_color='k')
...
def make_edge_label(edge, graph, annotators, edge_label_style='weight', normalization=lambda x: x,
    ...
    if edge_label_style == 'weight':
        ...
        values = [str(int(summary_statistic(judgments[annotator]))) if annotator in judgments else '-' for annotator in
                  annotators]
        label = '/'.join(values)
    else:
