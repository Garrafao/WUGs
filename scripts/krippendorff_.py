...
def alpha(reliability_data: Optional[Iterable[Any]] = None, value_counts: Optional[np.ndarray] = None,
    ...
    if value_counts is None:
        ...
        if value_domain is None:
            ...
            assert np.isin(reliability_data[~np.isnan(reliability_data)], value_domain).all(), \
    ...
    if expected is None:
        ...
        n_v_expected = expected*n_v.sum() # scale
        e = _random_coincidences(n_v_expected, dtype=dtype)
        
    d = _distances(value_domain, distance_metric, n_v, dtype=dtype)
    #print('Do', '\n', (0.5/n_v.sum())*(o * d).sum()) # trying to reconstruct the terms Do and De, please verify
    #print('De', '\n', (0.5/n_v.sum())*(e * d).sum())
